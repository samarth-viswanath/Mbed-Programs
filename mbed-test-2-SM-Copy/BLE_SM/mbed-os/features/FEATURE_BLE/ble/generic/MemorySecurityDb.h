...
public:
    ...
    virtual void set_entry_local_ltk(
        ...
        if (entry) {
			...
			const uint8_t* ltkData = ltk.data();
			printf("LTK set_entry_local_ltk -  Data stored: ");
			for (int i = 0; i < ltk.size(); i++) {
				printf("%02x", ltkData[i]);
			}
			printf("\n\r");
        }
    }
    ...
    virtual void set_entry_peer_ltk(
        ...
        if (entry) {
			...
			const uint8_t* ltkData = ltk.data();
			printf("LTK set_entry_peer_ltk-  Data stored: ");
			for (int i = 0; i < ltk.size(); i++) {
				printf("%02x", ltkData[i]);
			}
			printf("\n\r");
    ...
    virtual void set_entry_peer_irk(
		...
		printf( "MemSecdb:set_entry_peer_irk: In \n\r ");
        entry_t *entry = as_entry(db_handle);
        if (entry) {
			printf("MemSecdb:set_entry_peer_irk: In entry True\n\r");
            entry->peer_identity.irk = irk;
			printf("entry->peer_identity.irk = %d \n\r", entry->peer_identity.irk);
			printf(" irk = %d \n\r", &irk);
            entry->flags.irk_stored = true;
        }
		printf( "MemSecdb:set_entry_peer_irk: Out \n\r ");
    }
    ...
    virtual void set_entry_peer_bdaddr(
		...
		printf(" Mem Security Db : set_entry_peer_bdaddr: In \n\r");
        entry_t *entry = as_entry(db_handle);
        if (entry) {
            entry->peer_identity.identity_address = peer_address;
			//printf(" peer_address = %d", peer_address);
           ...
           peer_address[5], peer_address[4], peer_address[3], peer_address[2], peer_address[1], peer_address[0]);
            entry->peer_identity.identity_address_is_public = address_is_public;
						printf(" Address_is_public %d\r\n",address_is_public);
        }
		printf(" Mem Security Db : set_entry_peer_bdaddr: Out \n\r");
    }
...
private:
    ...
    virtual SecurityEntryKeys_t* read_in_entry_peer_keys(entry_handle_t db_entry) {
        entry_t *entry = reinterpret_cast<entry_t*>(db_entry);
		const uint8_t* ltkData = entry->peer_keys.ltk.data();
		printf("READ PEER KEYS  read_in_entry_peer_keys - LTK Data stored: ");
		for (int i = 0; i < entry->peer_keys.ltk.size(); i++) {
			printf("%02x", ltkData[i]);
		}
		printf("\n\r");
    ...
    virtual SecurityEntryKeys_t* read_in_entry_local_keys(entry_handle_t db_entry) {
        entry_t *entry = reinterpret_cast<entry_t*>(db_entry);
		const uint8_t* ltkData = entry->local_keys.ltk.data();
		printf("READ PEER KEYS  read_in_entry_local_keys - LTK Data stored: ");
		for (int i = 0; i < entry->local_keys.ltk.size(); i++) {
			printf("%02x", ltkData[i]);
		}
		printf("\n\r");
