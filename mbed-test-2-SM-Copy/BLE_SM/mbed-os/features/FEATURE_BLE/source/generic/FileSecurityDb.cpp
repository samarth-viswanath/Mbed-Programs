...
void FileSecurityDb::set_entry_peer_irk(
    ...
    printf("\n\ r set_entry_peer_irk IN\n\r");
	entry_t *entry = as_entry(db_handle);
    if (!entry) {
        printf("\n\ r set_entry_peer_irk NO ENTRY\n\r");
		return;
    }

    entry->flags.irk_stored = true;

    printf("\n\ r set_entry_peer_irk out\n\r");
...
void FileSecurityDb::set_entry_peer_bdaddr(
	...
	printf(" FileSecurityDb::set_entry_peer_bdaddr In \n\r");
    entry_t *entry = as_entry(db_handle);
    if (!entry) {
        return;
    }
	...
	printf(" FileSecurityDb::set_entry_peer_bdaddr Out \n\r");
}
...
void FileSecurityDb::restore() {
	...
	printf(" entry->flags->csrk_stored %d  \n\r",_entries[i].flags.csrk_stored);
    printf(" entry->flags->ltk_stored %d  \n\r",_entries[i].flags.ltk_stored);
	printf(" entry->flags->ltk_sent %d  \n\r",_entries[i].flags.ltk_sent);
	printf(" entry->flags->irk_stored %d  \n\r",_entries[i].flags.irk_stored);
...
void FileSecurityDb::sync(entry_handle_t db_handle) {
	...
	printf(" entry->flags->csrk_stored %d  \n\r",entry->flags.csrk_stored);
    printf(" entry->flags->ltk_stored %d  \n\r",entry->flags.ltk_stored);
	printf(" entry->flags->ltk_sent %d  \n\r",entry->flags.ltk_sent);
	printf(" entry->flags->irk_stored %d  \n\r",entry->flags.irk_stored);
