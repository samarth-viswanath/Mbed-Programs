...
ble_error_t GenericSecurityManager<TPalSecurityManager, SigningMonitor>::init_(
	...
	printf(" GenericSecurityManager ::init_ In \n\r");
	...
	printf(" GenericSecurityManager ::init_ Out \n\r");
    return result;
}
...
ble_error_t GenericSecurityManager<TPalSecurityManager, SigningMonitor>::acceptPairingRequest_(connection_handle_t connection) {
	...
	printf("link_authentication %d\n\r", link_authentication.value());
...
ble_error_t GenericSecurityManager<TPalSecurityManager, SigningMonitor>::setLinkSecurity_(
	...
	printf(" Gen Sec Manager : setLinkSecurity_ In \n\r");
    if (!_db) return BLE_ERROR_INITIALIZATION_INCOMPLETE;
    ControlBlock_t *cb = get_control_block(connection);
    if (!cb) {
        return BLE_ERROR_INVALID_PARAM;
	...
	printf(" Gen Sec Manager : setLinkSecurity_ Out \n\r");
}
...
ble_error_t GenericSecurityManager<TPalSecurityManager, SigningMonitor>::init_database(
    ...
    delete _db;
	printf(" Generic Sec man init_database :In \n\r");
/*#if BLE_SECURITY_DATABASE_FILESYSTEM
    ...
    if (KVStoreSecurityDb::open_db()) {
		printf(" KVStoreSecurityDb \n\r");
        _db = new (std::nothrow) KVStoreSecurityDb();
    } else
#endif
    {	
		printf(" MemorySecurityDb \n\r");
...
ble_error_t GenericSecurityManager<TPalSecurityManager, SigningMonitor>::enable_encryption(connection_handle_t connection) {
    if (!_db) return BLE_ERROR_INITIALIZATION_INCOMPLETE;
    ControlBlock_t *cb = get_control_block(connection);
	printf("enable_encryption:control block in \n\r");
    if (!cb) {
        return BLE_ERROR_INVALID_PARAM;
    }
	printf("enable_encryption:control block Out/getdistributionflags In \n\r");
    SecurityDistributionFlags_t* flags = _db->get_distribution_flags(cb->db_entry);
    if (!flags) {
        return BLE_ERROR_INVALID_PARAM;
    }
	printf("enable_encryption:getdistributionflags out / cb->masterIn \n\r");
    if (cb->is_master) {
        if (flags->ltk_stored) {
			...
			printf("enable_encryption:b->masterIn : flags->ltk_storedis True\n\r");
            return BLE_ERROR_NONE;
        } else {
			printf("enable_encryption:b->masterIn :flags->ltk_stored is False requestPairing(Connection) is True\n\r");
            return requestPairing(connection);
        }
    } else {
		printf("enable_encryption:cb->master Out/ cb->is_master is false / return slave_security_req(conn) \n\r");
        return slave_security_request(connection);
    }
}
...
void GenericSecurityManager<TPalSecurityManager, SigningMonitor>::enable_encryption_cb(
	...
	printf("enable_encryption_cb:control block in \n\r");
    if (!cb) {
        return;
    }
	printf("enable_encryption_cb:control block out/ distribution flags In \n\r");
    SecurityDistributionFlags_t* flags = _db->get_distribution_flags(cb->db_entry);
    if (!flags) {
        return;
    }
	printf("enable_encryption_cb:distribution flags Out/Entry keys In \n\r");
    if (entryKeys) {
		printf(" entryKeys is True \n\r");
        if (flags->secure_connections_paired) {
			printf(" flags->secure_connections_paired is True \n\r");
            _pal.enable_encryption(cb->connection, entryKeys->ltk, flags->ltk_mitm_protected);
        } else {
			printf(" flags->secure_connections_paired is False \n\r");
            _pal.enable_encryption(cb->connection, entryKeys->ltk, entryKeys->rand, entryKeys->ediv, flags->ltk_mitm_protected);
        }
    }
	printf("enable_encryption_cb: distribution flags Out/Entry keys Out \n\r");
}
...
void GenericSecurityManager<TPalSecurityManager, SigningMonitor>::on_link_encryption_result_(
	...
	printf(" on_link_encryption_result_ : in \n\r");
    ControlBlock_t *cb = get_control_block(connection);
    if (!cb) {
        return;
    }
	...
	printf(" on_link_encryption_result_ : Out");
...
void GenericSecurityManager<TPalSecurityManager, SigningMonitor>::on_keys_distributed_ltk_(
    ...
    MBED_ASSERT(_db);
	printf(" on_keys_distributed_ltk_ In\n\r ");
    ControlBlock_t *cb = get_control_block(connection);
    if (!cb) {
		printf(" on_keys_distributed_ltk_ In return 1 \n\r ");
        return;
    }

    SecurityDistributionFlags_t* flags = _db->get_distribution_flags(cb->db_entry);
    if (!flags) {
		printf(" on_keys_distributed_ltk_ In return 2 \n\r ");
        return;
    }

    flags->ltk_mitm_protected = cb->mitm_performed;
	printf(" on_keys_distributed_ltk_ Out\n\r ");
    _db->set_entry_peer_ltk(cb->db_entry, ltk);
}
...
void GenericSecurityManager<TPalSecurityManager, SigningMonitor>::on_keys_distributed_irk_(
    ...
    MBED_ASSERT(_db);
    ControlBlock_t *cb = get_control_block(connection);
	printf("on_keys_distributed_irk_ :In \n\r");
	printf("on_keys_distributed_irk_ : cb = %d \n\r",cb);
    if (!cb) {
        return;
    }

    SecurityDistributionFlags_t* flags = _db->get_distribution_flags(cb->db_entry);
	printf("on_keys_distributed_irk_ : flags = %d \n\r",flags);
    if (!flags) {
        return;
    }
	printf("on_keys_distributed_irk_ :Out \n\r");
...
void GenericSecurityManager<TPalSecurityManager, SigningMonitor>::on_keys_distributed_bdaddr_(
    ...
    MBED_ASSERT(_db);
    ControlBlock_t *cb = get_control_block(connection);
	printf(" on_keys_distributed_bdaddr_ : In \n\r");
    if (!cb) {
        return;
    }
	...
	printf(" on_keys_distributed_bdaddr_ : Out \n\r");
}
...
void GenericSecurityManager<TPalSecurityManager, SigningMonitor>::on_ltk_request_(
	...
	printf(" Generic Sec Manager : on_ltk_request_ 3 paarams: In \n\r");
    MBED_ASSERT(_db);
    ControlBlock_t *cb = get_control_block(connection);
    if (!cb) {
        return;
	...
	printf(" Generic Sec Manager : on_ltk_request_ 3 paramas: Out \n\r");
}
...
{
	printf("on_ltk_request_ 1 param: In \n\r");
    MBED_ASSERT(_db);
    ControlBlock_t *cb = get_control_block(connection);
    if (!cb) {
        return;
	...
	printf("on_ltk_request_ 1 param: Out \n\r");
}
